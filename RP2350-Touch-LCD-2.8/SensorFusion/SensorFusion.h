...
namespace sf
{
    ...
    // Moved outside SensorFusion due to GCC limitation
    struct Config
    {
    ...
    public:
        ...
        Output output() const
        {
            ...
            if (windValid_)
            {
                ...
                if (o.windBearing < 0)
                    o.windBearing += 2.0f * 3.14159265358979f;
            }
    ...
    private:
        ...
        Vec3 gpsToNED(double lat, double lon, float alt) const
        {
            ...
            float e = static_cast<float>(dLon * (rPrimeVert_ + refAlt_) * std::cos(refLat_ * DEG2RAD));
            float d = -(alt - refAlt_);
            return {n, e, d};
        }
        ...
        void nedToGps(const Vec3 &ned, double &lat, double &lon, float &alt) const
        {
            ...
            lon = refLon_ + (ned.y / ((rPrimeVert_ + refAlt_) * std::cos(refLat_ * DEG2RAD))) / DEG2RAD;
            alt = refAlt_ - ned.z;
        }
    };
