...
:  i2c_dev(i2c_dev)
{
    ...
    i2c_dev->write(ADDR, buf, 2, false);
...
bool BMP280::devicePresent(){
    uint8_t reg = REG_ID;
    uint8_t id(0);
    i2c_dev->write(ADDR, &reg, 1, true);  // true to keep master control of bus
    i2c_dev->read(ADDR, &id, 1, false);  // false - finished with bus
    return id == 0x58;
}
 ...
 bool BMP280::isMeasuring(){
    ...
    uint8_t status(0);
    i2c_dev->write(ADDR, &reg, 1, true);  // true to keep master control of bus
    i2c_dev->read(ADDR, &status, 1, false);  // false - finished with bus
    return (status & 0x08) != 0;
}
 bool BMP280::isImUpdate(){
    uint8_t reg = REG_STATUS;
    uint8_t status(0);
    i2c_dev->write(ADDR, &reg, 1, true);  // true to keep master control of bus
    i2c_dev->read(ADDR, &status, 6, false);  // false - finished with bus
    return (status & 0x01) != 0;
}

void BMP280::control(PowerMode powerMode, Oversampling temp, Oversampling pressure){
     uint8_t buf[2];

     // osrs_t x1, osrs_p x4, normal mode operation
    const uint8_t reg_ctrl_meas_val = (uint8_t(temp) << 5) | (uint8_t(pressure) << 2) | (uint8_t(powerMode));
    buf[0] = REG_CTRL_MEAS;
    buf[1] = reg_ctrl_meas_val;
    i2c_dev->write( ADDR, buf, 2, false);
}

void BMP280::config(StandbyTime standby, IIR filter){
    uint8_t buf[2];

    const uint8_t reg_config_val = ((uint8_t(standby) << 5) | (uint8_t(filter) << 2)) & 0xFC;
...
void BMP280::readRaw(int32_t* temp, int32_t* pressure) {
    ...
    i2c_dev->write(ADDR, &reg, 1, true);  // true to keep master control of bus
    i2c_dev->read(ADDR, buf, 6, false);  // false - finished with bus
...
void BMP280::read(){
    readRaw(&raw_temperature, &raw_pressure);
}

// intermediate function that calculates the fine resolution temperature
// used for both pressure and temperature conversions
int32_t BMP280::convert(int32_t temp, const BMP280::calib_param* params) {
...
int32_t BMP280::convert_temp(int32_t temp, const BMP280::calib_param* params) {
    // uses the BMP280 calibration parameters to compensate the temperature value read from its registers
    int32_t t_fine = convert(temp, params);
    return (t_fine * 5 + 128) >> 8;
}

int32_t BMP280::convert_pressure(int32_t pressure, int32_t temp, const BMP280::calib_param* params) {
...
void BMP280::get_calib_params(BMP280::calib_param* params) {
    ...
    i2c_dev->read(ADDR, buf, NUM_CALIB_PARAMS, false);  // false, we're done reading
...
int32_t BMP280::temperature(){
    return convert_temp(raw_temperature, &calib);
}
 
int32_t BMP280::pressure() {
    return convert_pressure(raw_pressure, raw_temperature, &calib);
}
